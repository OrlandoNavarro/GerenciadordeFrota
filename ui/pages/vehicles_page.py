...
def render():
    ...
    with tabs[1]:
        ...
        # Paginação
        rows_all = [v.to_dict() for v in vehicles]
        page_size = 10
        page_key = 'page_vehicle'
        ...
        if total_pages > 1:
            ...
            for i, c in enumerate(cols_pag):
                with c:
                    if st.button(str(i + 1), key=f'page_vehicle_btn_{i+1}'):
        ...
        render_table([v.to_dict() for v in paginated], columns=['id', 'placa', 'modelo', 'marca', 'ano', 'status', 'transporter_id'], entity='vehicle')
        ...
        # edição inline direta quando edit_id presente
        if edit_id is not None:
            ...
            if vehicle:
                ...
                with st.form('edit_vehicle_form_inline'):
